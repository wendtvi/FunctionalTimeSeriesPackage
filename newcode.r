...
#			thetahat = thetahat,
	...
	# 
	# points(z1[regimes>0], z2[regimes>0], pch = 19, col = rgb(.2,0,.7,.9), cex=.5)
	
detach(foo)

foo = mcsimu(n = 200, sigma1 = 2, sigma2 = 1, q = .5, p = 20)
...
# 20 PRIMEIRAS CURVAS OBSERVADAS
# col = c(rgb(.7,0,.2,1), rgb(.2,0,.7,1))
# for (t in 1:20) plot(u, Y[,t], type = 'l', col = col[1+theta[t]])
