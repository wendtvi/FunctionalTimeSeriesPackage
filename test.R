...
set.seed(4)
n=200 # sample size
sigma1=.8 # variance of first error term # ?? VARIAR DE 1 a 2 NUMA GRADE COM 11 PONTOS seq(from=1,to=2,length=11)
sigma2=1.2 # variance of second error term # ?? VARIAR DE 0.5 a 1 NUMA GRADE COM 11 PONTOS seq(from=.5,to=1,length=11)
...
foon = super_fun(u, Y_eval, dimension = 2, p = p)
...
z1hat = etahat[1,]
z1hat.lag = c(0,etahat[1,1:(n-1)]) # acrscento um zero na primeira posicao da serie defasada
z2hat = etahat[2,]
z2hat.lag = c(0,etahat[2,1:(n-1)])
...
plot(z1hat,z2hat, pch = 20, type = 'n')
points(z1hat[theta<1], z2hat[theta<1], pch = 19, col = rgb(.7,0,.2,.5),cex=1.4)
points(z1hat[theta>0], z2hat[theta>0], pch = 19, col = rgb(.2,0,.7,.5),cex=1.4)
points(z1hat[thetahat.fts>0], z2hat[thetahat.fts>0], pch = 19, col = rgb(.2,0,.7,.9), cex=.5)
...
# points(z1[thetahat.fts>0], z2[thetahat.fts>0], pch = 19, col = rgb(.2,0,.7,.9), cex=.5)
detach(foon)
